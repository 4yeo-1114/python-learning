"""
============================================================
Day 3 练习题：文件操作 + 字符串 + 异常处理
============================================================
答案在 day03_solution.py，做完再看！
"""

# ============================================================
# 练习 1：字符串方法 — 姓名格式化
# 写一个函数 format_name(full_name)，输入 "  张三  " 或 "zhang san"
# 输出 "Zhang San"（首字母大写，去多余空格）
# 提示：strip(), title()
# ============================================================
...
# 有一行数据 "101,张三,20,计算机科学,北京"
# 请编写代码：
#   a) 按逗号分割
#   b) 用 " | " 连接起来，输出 "101 | 张三 | 20 | 计算机科学 | 北京"
# ============================================================

# TODO: 在这里写你的代码

s = "101,张三,20,计算机科学,北京"
parts = s.split(',')
csv = " | ".join(parts)
...
# ============================================================
# 练习 3：文件写入 — 生成学生数据文件
# 写一个函数 generate_student_file(filename, students)
# students 是一个列表，每个元素是 {"id": ..., "name": ..., "score": ...}
# 把每个学生的信息写成一行，格式：学号,姓名,分数
# 提示：用 with open(..., "w", encoding="utf-8") as f
# ============================================================
...
# ============================================================
# 练习 4：文件读取 — 词频统计器
# 写一个函数 count_words(filename)
# 读取文件内容，统计每个单词出现的次数（忽略大小写）
# 返回一个字典，如 {"hello": 3, "world": 2, ...}
# 提示：
#   - 用 .lower() 忽略大小写
#   - 用 .split() 分割单词
#   - 考虑去掉标点符号（简单版：只统计字母数字）
# ============================================================
...
# ============================================================
# 练习 5：异常处理 — 安全的数字输入
# 写一个函数 get_number(prompt)
# 反复提示用户输入，直到输入的是合法数字
# 如果输入不合法，打印 "请输入有效的数字！" 并继续
# 返回 float 类型
# 提示：用 try/except ValueError + while 循环
# ============================================================
...
# ============================================================
# 练习 6：异常处理 — 安全的文件读取器
# 写一个函数 read_file_safely(filepath)
# 尝试读取文件，处理以下情况：
#   - 文件不存在：打印提示，返回 None
#   - 没有权限：打印提示，返回 None
#   - 编码错误（UnicodeDecodeError）：用 gbk 编码重试
#   - 其他错误：打印错误信息，返回 None
# 提示：try/except 可以有多个 except 分支
# ============================================================
...
# ============================================================
# 练习 7（综合挑战）：文件版通讯录升级
# 在 Day 2 通讯录的基础上，加上文件持久化功能：
#
#   1. add_contact(contacts, name, phone, email)
#      添加联系人后自动保存到文件
#
#   2. load_contacts(filepath)
#      程序启动时从文件加载通讯录
#      如果文件不存在，返回空列表
#      文件格式每行: name,phone,email
#
#   3. 主程序：一个简单的命令行菜单
#      1. 查看通讯录
#      2. 添加联系人
...
#      每次操作后自动保存
#
#   4. contact 字典增加一个 id 字段（自动生成，递增）
# ============================================================
...
def delete_contact(contacts,contact_id):
    ...
    for i,c in enumerate(contacts):
        if c["id"] == contact_id:
            ...
            save_contacts(contacts)
            print(f"[ok] 已删除：{remove["name"]}")
            return True
    print(f"[失败] 未找到ID:{contact_id}")
    return False
...
print("\n[Congrats!] Day 3 练习完成！对比答案看 day03_solution.py")
